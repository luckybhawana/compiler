       ...
       // declarationsParser.parse(token, routineId);
        ...
        // Missing BEGIN: Attempt to parse anyway if possible.
            ...
            if (StatementParser.STMT_START_SET.contains(tokenType)) {
                ...
                statementParser.parseList(token, rootNode, RIGHT_BRACE, MISSING_LEFTBRACE);
            }
        ...
        if((token.getType() == INT || token.getType() == REAL))
        {
        	declarationsParser.parse(token, routineId);
        }
            
        //TokenType tokenTy = token.getType();
        
        statementParser.parseList(token, rootNode, LEFT_BRACE, MISSING_LEFTBRACE);
        */
